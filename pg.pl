...
inicio1:-
	...
	expand_file_name('Bases_de_Datos/*.rdf',A),
	rdf_load([A],[format(xml)]).		  % carga la informacion de todos los libros

inicio:-
	[library(rdf)],
	use_module(library(semweb/rdf_db)),
	expand_file_name('Bases_de_Datos/*.rdf',A),		   % carga la informacion de todos los libros
	expand_file_name('libros.rdf',E),  % carga la informacion de tipo
